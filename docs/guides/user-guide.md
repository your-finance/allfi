...
1. **设置密码**
   - 首次访问时，系统会引导你设置密码
   - 支持两种密码模式：
     - **PIN 码**：4-20 位纯数字，简单便捷
     - **复杂密码**：8-20 位，必须包含大小写字母和数字
   - 密码经 bcrypt 加密存储，安全可靠
...
| 清除缓存 | 清除价格和汇率缓存 |
| 修改密码 | 更改登录密码 |
| 切换密码类型 | 在 PIN 码和复杂密码之间切换 |

**密码类型切换**：
1. 进入「设置」→「安全设置」
2. 点击「切换密码类型」
3. 输入当前密码验证身份
4. 选择新的密码类型（PIN 或复杂密码）
5. 设置新密码
6. 若已启用 2FA，还需输入 6 位验证码
...
当前版本需要重置数据库来重新设置密码。备份数据后删除 `allfi.db` 文件，重启服务即可重新设置。

### Q6: 如何切换密码类型？

在「设置」→「安全设置」中可以随时在 PIN 码和复杂密码之间切换。切换时需要验证当前密码，如果启用了 2FA 还需输入验证码。
