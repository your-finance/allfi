...
export const assetService = {
  ...
  async getSummary() {
    ...
    const totalValue =
      summaryData.total_value || cexTotal + blockchainTotal + manualTotal;
    ...
    return {
      ...
      categories: {
        cex: {
          ...
          percentage: totalValue ? (cexTotal / totalValue) * 100 : 0,
        },
        blockchain: {
          value: blockchainTotal,
          count: walletAddresses.length,
          percentage: totalValue ? (blockchainTotal / totalValue) * 100 : 0,
        },
        manual: {
          value: manualTotal,
          count: manualAssets.length,
          percentage: totalValue ? (manualTotal / totalValue) * 100 : 0,
        },
      },
      cexAccounts,
      walletAddresses,
...
export const rateService = {
  ...
  async getHistoryRates(from, to, range = "30D") {
    ...
    const result = await get("/rates/history", {
      base: from,
      quote: to,
  ...
  async getPrices(symbols = "BTC,ETH") {
    if (USE_MOCK) {
      ...
      const mockPrices = {
        BTC: {
          price_usd: 96000 + (Math.random() - 0.5) * 2000,
          change_24h: (Math.random() - 0.4) * 5,
        },
        ETH: {
          price_usd: 2300 + (Math.random() - 0.5) * 200,
          change_24h: (Math.random() - 0.4) * 6,
        },
        USDC: { price_usd: 1.0, change_24h: 0 },
        USDT: { price_usd: 1.0, change_24h: 0 },
        BNB: {
          price_usd: 420 + (Math.random() - 0.5) * 40,
          change_24h: (Math.random() - 0.4) * 4,
        },
        SOL: {
          price_usd: 180 + (Math.random() - 0.5) * 30,
          change_24h: (Math.random() - 0.4) * 7,
        },
      };
        ...
        keys: [
          {
            ...
            masked_key: "",
            description:
              "以太坊区块链浏览器 API（余额查询、Gas 价格、交易记录）",
          },
          {
            provider: "bscscan",
            ...
            configured: false,
            masked_key: "",
            description: "BSC 区块链浏览器 API（余额查询、Gas 价格、交易记录）",
          },
          {
            provider: "coingecko",
            ...
            configured: false,
            masked_key: "",
            description: "加密货币价格 API（可选，免费 API 无需 Key）",
          },
          {
            ...
            configured: false,
            masked_key: "",
            description: "汇率查询 API（可选，免费 API 无需 Key）",
          },
        ],
      };
    }
...
export const analyticsService = {
  ...
  async getDailyPnL(range = "30d") {
    if (USE_MOCK) {
      await simulateDelay(300);
      const daysCount =
        { "7d": 7, "30d": 30, "90d": 90, "1y": 365 }[range] || 30;
      const daily = [];
        ...
        details: [
          {
            ...
            weight: 0.25,
            description: "单个资产占比偏高",
            suggestion: "考虑分散投资，建议将最大单个资产占比控制在 30% 以下",
          },
          {
            category: "平台风险",
            score: 70 + Math.random() * 25,
            weight: 0.25,
            description: "资产分散在多个平台",
            suggestion: "继续保持平台多元化",
          },
          {
            category: "波动性风险",
            score: 60 + Math.random() * 30,
            weight: 0.25,
            description: "组合波动率处于中等水平",
            suggestion: "可适度增加稳定币配置以降低风险",
          },
          {
            category: "缓冲能力",
