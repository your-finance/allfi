...
export function refreshCexAccount(id) {
  ...
  if (index !== -1) {
    ...
    accounts[index].balance = Math.round(
      accounts[index].balance * (1 + randomChange),
    );
...
export function refreshWalletAddress(id) {
  ...
  if (index !== -1) {
    const randomChange = (Math.random() - 0.5) * 0.1;
    wallets[index].balance = Math.round(
      wallets[index].balance * (1 + randomChange),
    );
...
export function getAssetSummary() {
  ...
  // 计算链上总值
  const blockchainTotal = walletAddresses.reduce(
    (sum, w) => sum + w.balance,
    0,
  );
  ...
  const points =
    timeRange === "7D"
      ...
      : timeRange === "30D"
        ...
        : timeRange === "90D"
          ? 90
          : timeRange === "LAST_YEAR" || timeRange === "1Y"
            ? 365
            : 730;
 ...
 * 获取风险总览数据
 */
export function getRiskOverview(period = "30d") {
  return {
    volatility: 18.5, // 波动率 (%)
    sharpe_ratio: 1.42, // 夏普比率
    max_drawdown: -12.3, // 最大回撤 (%)
    var_95: -8.5, // 95% VaR (%)
    cvar_95: -11.2, // 95% CVaR (%)
    beta_btc: 0.85, // 相对 BTC 的 Beta
    beta_eth: 0.92, // 相对 ETH 的 Beta
    period,
  };
}
 ...
 * 获取风险指标历史趋势
 */
export function getRiskMetrics(period = "30d") {
  const points =
  ...
  for (let i = points; i >= 0; i--) {
    ...
    // 模拟波动率在 15-25% 之间波动
    volatility.push(20 + Math.sin(i * 0.2) * 5 + (Math.random() - 0.5) * 2);
    // 模拟夏普比率在 0.8-2.0 之间波动
    sharpe.push(1.4 + Math.cos(i * 0.15) * 0.4 + (Math.random() - 0.5) * 0.2);
  }
 ...
 * 获取回撤曲线数据
 */
export function getDrawdown(period = "30d") {
  const points =
    period === "7d" ? 7 : period === "30d" ? 30 : period === "90d" ? 90 : 365;
  const labels = [];
  const drawdown = [];
  ...
  for (let i = points; i >= 0; i--) {
    ...
    // 模拟资产价值波动
    currentValue = currentValue * (1 + (Math.random() - 0.48) * 0.03);
    if (currentValue > maxValue) maxValue = currentValue;

    // 计算回撤百分比
    const dd = ((currentValue - maxValue) / maxValue) * 100;
    drawdown.push(dd);
  }
 ...
 * 获取 Beta 系数对比数据
 */
export function getBeta(period = "30d") {
  return {
    portfolio_beta: 0.85,
    benchmarks: [
      { name: "BTC", beta: 1.0, correlation: 0.78 },
      { name: "ETH", beta: 1.0, correlation: 0.82 },
      { name: "S&P 500", beta: 0.15, correlation: 0.25 },
    ],
  };
}
